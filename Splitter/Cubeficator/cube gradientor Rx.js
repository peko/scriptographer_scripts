...
for (si in items) {
    ...
    if (i.children.length == 3) {
        ...
        s.fillColor = new GradientColor(gr, new Point(c.x-w  , c.y-r/2), new Point(c.x+w  , c.y-r/2));
        
        s = i.children[1];
        var gr = new Gradient() {type: gt, stops:[new GradientStop(c2, 0), new GradientStop(c3, 1)]};
        s.fillColor = new GradientColor(gr, new Point(c.x  , c.y+r), new Point(c.x+w  , c.y-r/2));
        
        s = i.children[2];
        var gr = new Gradient() {type: gt, stops:[new GradientStop(c3, 0), new GradientStop(c1, 1)]};
        s.fillColor = new GradientColor(gr, new Point(c.x-w  , c.y-r/2), new Point(c.x  , c.y+r));
    } else print("NOT CUBE");
